...
class DriveFileController extends Controller
{
    ...
    public function preview(DriveFile $driveFile, Request $request)
    {
        return redirect()->away(
            $this->drive->forUser($request->user())->getPreviewUrl($driveFile->google_file_id)
        );
    }

    public function download(DriveFile $driveFile, Request $request)
    {
        return redirect()->away(
            $this->drive->forUser($request->user())->getTemporaryUrl($driveFile->google_file_id, 3600)
        );
    }

    public function destroy(DriveFile $driveFile, Request $request, DeleteDriveFileAction $action): JsonResponse
    {
        $action->execute($driveFile, $request->user());
    ...
    {
        return new DriveFileDetailResource($action->execute($driveFile, $request->user()));
    }
